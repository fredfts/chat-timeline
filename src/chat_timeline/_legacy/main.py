...
def format_tool_call_detail(tc):
    ...
    def _get_file_param(p):
        return relative_path(
            p.get("relativeWorkspacePath") or p.get("filePath")
            or p.get("file_path") or p.get("path") or ""
        )
    ...
    # Claude Code tools
    if name == "Read":
        return f"{name}: {relative_path(params.get('file_path', ''))}"
...
def export_chat_markdown(meta, turns, include_tool_params=False):
    ...
    meta: dict with standardized keys (name, id, created, last_updated, status,
          mode, model, max_mode, agent_backend, branch, context_tokens,
          context_limit, lines_added, lines_removed, files_changed,
          is_agentic, files_affected, source)
    turns: list of turn dicts (common format from build_conversation)
    """
    lines = []
...
def parse_chat_export(filepath):
    ...
    for line in body.splitlines():
        ...
        if qm:
            if tb and cur:
                cur["thinking_blocks"].append(tb); tb = None
            if cur:
                turns.append(cur)
            cur = dict(number=int(qm.group(1)), user_model=qm.group(2),
                       user_timestamp=qm.group(3), user_text="",
                       thinking_blocks=[], tool_calls=[],
                       response_model="", response_timestamp="",
                       response_text="")
            section = "q"
            continue
        ...
        if section == "q":
            ...
            if thm:
                ...
                tb = dict(timestamp=thm.group(1),
                          duration_s=float(thm.group(2)), text="")
            elif tb is not None:
                if line.strip():
                    tb["text"] += line + "\n"
            ...
            if tcm:
                cur["tool_calls"].append(dict(
                    detail=tcm.group(1), status=tcm.group(2),
                    timestamp=tcm.group(3)))
...
def get_staged_files():
    out, _ = git_run("diff", "--cached", "--name-status")
    return out


def get_unstaged_files():
    out, _ = git_run("diff", "--name-status")
    return out


def get_untracked_files():
    out, _ = git_run("ls-files", "--others", "--exclude-standard")
    return out


def get_head_hash():
    out, _ = git_run("rev-parse", "HEAD")
    return out


def get_head_short():
    out, _ = git_run("rev-parse", "--short", "HEAD")
    return out


def get_current_branch():
    out, _ = git_run("rev-parse", "--abbrev-ref", "HEAD")
    return out


def get_head_message():
    out, _ = git_run("log", "-1", "--format=%B")
    return out.strip()


def get_head_date():
    out, _ = git_run("log", "-1", "--format=%aI")
    return out.strip()
